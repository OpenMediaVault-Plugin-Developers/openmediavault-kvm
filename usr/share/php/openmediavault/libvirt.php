...
class Libvirt {
  ...
  // added from VMBashboard - https://github.com/VMDashboard/vmdashboard/blob/master/pages/libvirt.php
    ...
    function domain_snapshot_get_info($domain, $name) {
        $dom= $this->get_domain_object($domain);
        $tmp = $this->get_xpath($dom, '//domain/metadata/snapshot'.$name, false);
        $var = $tmp[0];
        unset($tmp);
        return $var;
    }
    ...
    function domain_snapshot_lookup_by_name($domain, $name) {
        $dom = $this->get_domain_object($domain);
        $tmp = libvirt_domain_snapshot_lookup_by_name($dom, $name);
        return ($tmp) ? $tmp : $this->_set_last_error();
    }

    // revert domain to snapshot state
    function domain_snapshot_revert($domain, $name) {
        $snap = $this->domain_snapshot_lookup_by_name($domain, $name);
        $tmp = libvirt_domain_snapshot_revert($snap);
        return ($tmp) ? $tmp : $this->_set_last_error();
    }

    // get domain snapshot xml info
    function domain_snapshot_get_xml($domain, $name) {
        $snap = $this->domain_snapshot_lookup_by_name($domain, $name);
        $tmp = libvirt_domain_snapshot_get_xml($snap);
        return ($tmp) ? $tmp : $this->_set_last_error();
    }
    ...
    function domain_set_autostart($res, $int = 0) {
        return (libvirt_domain_set_autostart($res, $int));
    }
    ...
    // added by omv-extras
    function get_storagevolume_by_path($path) {
        $tmp = libvirt_storagevolume_lookup_by_path($this->conn, $name);
        return ($tmp) ? $tmp : $this->_set_last_error();
    }
